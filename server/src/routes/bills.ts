...
import { formatBillNumber, getFinancialYearLabel, shopIdToKey } from "../services/billNumber.js";
...
router.get("/search", async (req, res) => {
  try {
    ...
    if (dateFrom || dateTo) {
      filter.createdAt = {};
      if (dateFrom && !isNaN(dateFrom.getTime())) (filter.createdAt as Record<string, Date>).$gte = dateFrom;
...
router.post("/", async (req, res) => {
  try {
    ...
    const invoiceDate = body.createdAt ? new Date(String(body.createdAt)) : new Date();
    const { billNumber, fyLabel } = await allocateNextBillNumber(shop, invoiceDate);
...
router.put("/:id", async (req, res) => {
  try {
    ...
    if (mode === "new") {
      const shop = (body.shop as "sivasakthi" | "meenatchi") || existing.shop;
      const invoiceDate = body.createdAt ? new Date(String(body.createdAt)) : new Date();
      const { billNumber } = await allocateNextBillNumber(shop, invoiceDate);
      const doc = await Bill.create({
        ...body,
        shop,
        billNumber,
    ...
    return res.status(400).json({ error: "Invalid mode; use overwrite or new" });
  } catch (e) {
    console.error(e);
...
router.get("/meta/autofill/customers", async (_req, res) => {
  try {
    const rows = await Bill.aggregate([
      {
        $match: {
          $or: [{ "customer.name": { $ne: "" } }, { "customer.mobile": { $ne: "" } }],
        },
      },
      {
        $group: {
          _id: { name: "$customer.name", mobile: "$customer.mobile", place: "$customer.place" },
          count: { $sum: 1 },
        },
      },
      { $sort: { count: -1 } },
